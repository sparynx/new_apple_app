...
function getTraceData() {
  ...
  const sentryTrace = span ? spanUtils.spanToTraceHeader(span) : tracing.generateSentryTraceHeader(traceId, undefined, sampled);
  ...
  const baggage$1 = baggage.dynamicSamplingContextToSentryBaggageHeader(dynamicSamplingContext$1);
  ...
  const validBaggage = isValidBaggageString(baggage$1);
  if (!validBaggage) {
    logger.logger.warn('Invalid baggage data. Not returning "baggage" value');
  }

  return {
    'sentry-trace': sentryTrace,
    ...(validBaggage && { baggage: baggage$1 }),
  };
}
