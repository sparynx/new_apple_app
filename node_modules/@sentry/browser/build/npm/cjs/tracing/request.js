...
function addTracingHeadersToXhrRequest(xhr, client, span) {
  ...
  const sentryTraceHeader =
    span && core.hasTracingEnabled() ? core.spanToTraceHeader(span) : core.generateSentryTraceHeader(traceId, spanId, sampled);

  const sentryBaggageHeader = core.dynamicSamplingContextToSentryBaggageHeader(
...
function setHeaderOnXhr(
  ...
  try {
    ...
    if (sentryBaggageHeader) {
      ...
      // eslint-disable-next-line @typescript-eslint/no-non-null-assertion
      xhr.setRequestHeader(core.BAGGAGE_HEADER_NAME, sentryBaggageHeader);
    }
  } catch (_) {
