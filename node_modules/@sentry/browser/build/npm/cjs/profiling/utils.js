...
function getTraceId(event) {
  ...
  if (typeof traceId === 'string' && traceId.length !== 32) {
    if (debugBuild.DEBUG_BUILD) {
      core.logger.log(`[Profiling] Invalid traceId: ${traceId} on profiled event`);
    }
  }
...
function convertJSSelfProfileToSampledFormat(input) {
  ...
  const origin =
    typeof performance.timeOrigin === 'number' ? performance.timeOrigin : core.browserPerformanceTimeOrigin || 0;
  const adjustForOriginChange = origin - (core.browserPerformanceTimeOrigin || origin);
...
function isValidSampleRate(rate) {
  ...
  if (rate < 0 || rate > 1) {
    debugBuild.DEBUG_BUILD && core.logger.warn(`[Profiling] Invalid sample rate. Sample rate must be between 0 and 1. Got ${rate}.`);
    return false;
  }
  return true;
}
...
function isValidProfile(profile) {
  ...
  if (!profile.frames.length) {
    if (debugBuild.DEBUG_BUILD) {
      core.logger.log('[Profiling] Discarding profile because it contains no frames');
    }
    return false;
  }
...
function startJSSelfProfile() {
  ...
  try {
    ...
    if (debugBuild.DEBUG_BUILD) {
      ...
      core.logger.log('[Profiling] Disabling profiling for current user session.');
    }
...
function shouldProfileSpan(span) {
  ...
  if (PROFILING_CONSTRUCTOR_FAILED) {
    if (debugBuild.DEBUG_BUILD) {
      core.logger.log('[Profiling] Profiling has been disabled for the duration of the current user session.');
    }
    return false;
  }
  ...
  if (!span.isRecording()) {
    if (debugBuild.DEBUG_BUILD) {
      core.logger.log('[Profiling] Discarding profile because transaction was not sampled.');
    }
    return false;
  }
  ...
  const options = client && client.getOptions();
  if (!options) {
    debugBuild.DEBUG_BUILD && core.logger.log('[Profiling] Profiling disabled, no options found.');
    return false;
  }
  ...
  if (!isValidSampleRate(profilesSampleRate)) {
    debugBuild.DEBUG_BUILD && core.logger.warn('[Profiling] Discarding profile because of invalid sample rate.');
    return false;
  }
